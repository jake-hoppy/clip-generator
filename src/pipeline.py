...
def run_whisper_rank(
    ...
    """
    Whisper gives full transcript; LLM chooses clip boundaries (start/end) and scores each.
    Extract those clips to data/candidates/<video_id>/, then top N to data/candidates_ranked/.
    """
    require_ffmpeg()
    ensure_data_dirs()
    ...
    for mpath in manifests_videos_dir().glob("*.json"):
        ...
        for seg in clips:
            ...
            seg["text"] = ""  # LLM doesn't return snippet; optional to fill from transcript later
            ...
            try:
                ...
                continue
            this_video_manifest_clips.append({k: v for k, v in seg.items() if k != "_video_path"})
            all_segments.append(seg)
