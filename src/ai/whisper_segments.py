...
def get_whisper_segments_raw(
    video_path: Path,
    video_id: str,
    model: str = "whisper-1",
) -> list[dict]:
    """
    Run Whisper API and return raw segments with no merging.
    Each segment has start_sec, end_sec, text, duration_seconds.
    Used when the LLM will decide clip boundaries from the full transcript.
    """
    if not video_path.exists():
        logger.warning("Video not found: %s", video_path)
        return []
    ...
    return _segments_to_dicts(result.segments)


def _segments_to_dicts(segments: list) -> list[dict]:
    """Convert Whisper API segment objects to list of dicts with start_sec, end_sec, text, duration_seconds."""
    out = []
    for s in segments:
...
def format_transcript_for_llm(segments: list[dict]) -> str:
    """Format segment list as a single transcript string with timestamps for the LLM."""
    lines = []
    for s in segments:
